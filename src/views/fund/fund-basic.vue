  ...
  <div class="chart-container">
    <el-row>
      ...
      >
        ...
        <el-button type="primary" @click="stopsend">停止估值</el-button></el-col>
    </el-row>
    ...
    <el-table :data="fundNavDetail" border stripe fit :cell-style="cellStyle">
      <el-table-column label="基金代码" width="100">
        ...
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>投资类型: {{ scope.row.fund_type }}</p>
            <p>成立日期: {{ scope.row.found_date }}</p>
            <p>管理人: {{ scope.row.management }}</p>
            <p>业绩比较基准: {{ scope.row.benchmark }}</p>
            <div slot="reference" class="name-wrapper">
              <div>{{ scope.row.ts_code }}</div>
              <div>{{ scope.row.name }}</div>
              <el-tag
                v-if="
                  scope.row.is_increase_count <= -3 &&
                    scope.row.change_count_percent_rank <= 0.2 &&
                    scope.row.calmar_ratio_rank >= 0.8
                "
                slot="reference"
              ...
              <el-tag
                v-else-if="
                  scope.row.is_increase_count > 3 &&
                    scope.row.is_increase_count >= 0.99 &&
                    scope.row.change_count_percent_rank <= 0.2
                "
                slot="reference"
                type="danger"
      ...
      <el-table-column
        ...
        label="近180日价格排名"
        sortable
        :formatter="pctRankFormatter"
      />
      <el-table-column prop="diff_pct_rank" label="变动排名" sortable>
        <template slot-scope="scope">
          <el-tag
            v-if="scope.row.diff_pct_rank <= 0"
            slot="reference"
            type="success"
          >{{ `${(scope.row.diff_pct_rank * 100).toFixed(2)}%` }}</el-tag>
          <el-tag
            v-else-if="scope.row.diff_pct_rank > 0"
            slot="reference"
            type="danger"
          >{{ `${(scope.row.diff_pct_rank * 100).toFixed(2)}%` }}</el-tag>
        </template>
      ...
      <el-table-column prop="is_increase_count" label="连续涨跌次数" sortable />
      <el-table-column
        prop="change_count_percent"
        label="当前涨跌次数出现概率"
        sortable
        :formatter="changeCountPercent"
      />
      ...
      <!-- <el-table-column label="其他信息">
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            ...
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">其他信息</el-tag>
            </div>
          </el-popover>
        </template>
      </el-table-column> -->
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <router-link
            :to="{
              name: 'FundsNav',
              query: { fundCode: scope.row.ts_code }
            }"
          >
...
export default {
  ...
  methods: {
    ...
    pctRankFormatter(row, column) {
      return `${(row.pct_rank * 100).toFixed(2)}%`
    },
    changeCountPercent(row, column) {
      return `${(row.change_count_percent * 100).toFixed(2)}%`
    }
  }
}
</script>
